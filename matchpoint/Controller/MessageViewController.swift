...
class MessageViewController: ChatViewController {
    ...
    override func viewDidLoad() {
        ...
        self.viewModel = MessageViewModel(bubbleStyle: BubbleStyle.facebook)
...
//        let url : String = BASE_URL+PLAYERS_URL+clubId+COURT_URL
            
        
        AF.request("http://192.168.178.20:8081/players/1/chats/1/messages").responseDecodable { (response: DataResponse<[MatchpointMessage], AFError>) in
            self.matchpointMessages = response.value!
            
            self.viewModel.messages.removeAll()

            for matchpointMes in self.matchpointMessages{
                
                let newChatMessage = Message(id: String(matchpointMes.id), sendByID: matchpointMes.sender.id, createdAt: Date.init(), text: matchpointMes.content)
                self.addMessage(newChatMessage)
            }
