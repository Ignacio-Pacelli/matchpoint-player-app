...
extension UIImageView {
	...
	func sample(_ topic: String, _ subtopic: String, _ index: Int) {
		...
		let options: KingfisherOptionsInfo = [.processor(processor), .scaleFactor(UIScreen.main.scale), .transition(.fade(0.5))]
    ...
    func loadPic(_ url : String) {
        ...
        self.kf.setImage(with: urlPic, placeholder: placeholder, options: options)
    }
}
